...
3. Data home is the `home=` value above (override `--home`); `home_source=` says how it was resolved (`cli`/`env` overrides, `pointer` = `$XDG_CONFIG_HOME`-aware pointer file, `default` = `~/job-search`). The first time a run touches the data home, say where it is and how it was chosen (e.g. "data home `~/job-search` (via pointer)") — where state lives must never be a surprise. All paths below are relative to it. `--home` is a top-level flag and must **precede** the subcommand on every script call (`jobs_db.py --home <dir> list`, never `jobs_db.py list --home <dir>`); when a turn makes several script calls, export `JOBSEARCH_HOME=<dir>` once instead of repeating `--home` on each.
4. If `config/settings.toml` is missing this is a fresh install. Interactive: tell the user the data home will be created at `home=`; if `home_source=default`, ask once (AskUserQuestion) whether to keep `~/job-search` or use another directory — on a custom answer, export `JOBSEARCH_HOME=<dir>` for the rest of the turn. Then run `python3 ${CLAUDE_SKILL_DIR}/scripts/doctor.py bootstrap` (it writes the pointer file so the choice sticks for future runs) and continue. Headless: bootstrap at the resolved `home=` and report what was created and where.

## 1. `setup`
Announce the data home and its source (§0.3–0.4, including the fresh-install question), then run `doctor.py bootstrap`, `doctor.py register-firecrawl-mcp`, then `doctor.py` and show the table. If `resume/` has no resume and `scoring.resume_url` is empty: create `resume/` (bootstrap does) and ask the user to drop a PDF/Markdown/text resume there **or** give a hosted URL (personal site or LinkedIn public profile) — headless: write this ask into the report and stop. Remind the user to edit `config/profile.md` and `config/cover-letter-style.md`, and to authorize Notion MCP once (`/mcp`) if `notion.enabled`. Offer scheduler templates from `assets/schedulers/` (see `references/headless.md`).
...
2. `python3 …/chrome_keep.py ensure` then attach Playwright over its CDP endpoint (never `launch_persistent_context` as a child of the fill process). Open one new tab per application (`browser_tabs new`, `browser_navigate`). Use `browser_snapshot` refs; `browser_fill_form` for known fields from `config/profile.md` + `resume.md`; `browser_file_upload` for `resume.pdf` (and cover letter when asked); screenshot each step into `evidence/`. **After fill, leave the headed tab open** — no `browser.close` / `browser_close` / killing Chrome. The user verifies and may click Submit; you still do not, unless `apply_guard.py` allows it.
...
- Never close the headed apply browser after fill (`chrome_keep.py` / `references/apply-flow.md`). The filled tab stays up for the user.
