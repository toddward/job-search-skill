...
6. **Score** each job that is `new` or has no score for the current `rubric_version`: `python3 …/fit_score.py --master resume/master.md --job <job.json> --config-json <scoring cfg> --age-days N`. Weights and red-flag caps: `references/scoring-rubric.md`. Add `fit_score`, `fit_breakdown` (the script output) and up to 3 short `fit_reasons` (plain-words "why it fits", written by you from the breakdown — never a different number) to the record JSON, then `jobs_db.py upsert-json` again so they persist.
