...
6. **Score** each job that is `new` or has no score for the current `rubric_version`: once per scan, `python3 …/config.py get scoring > scoring.json` (`--config-json` takes a **file path**, not inline JSON); then per job, `python3 …/fit_score.py --master resume/master.md --job <job.json> --config-json scoring.json --age-days N`. Weights and red-flag caps: `references/scoring-rubric.md`. Add `fit_score`, `fit_breakdown` (the script output) and up to 3 short `fit_reasons` (plain-words "why it fits", written by you from the breakdown — never a different number) to the record JSON, then `jobs_db.py upsert-json` again so they persist.
